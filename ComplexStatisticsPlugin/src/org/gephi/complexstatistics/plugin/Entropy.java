 ...
 */
public class Entropy implements Statistics, LongTask {
	...
	private void calculateValue(HierarchicalGraph graph) {
		...
		for (int k = 0; k < count && !cancel; k++) {
			double val = size[k] / (double)n;
			sum1 += val * Math.log(val);
			sum2 += Math.log(val);
		}
		value = sum1 / sum2;
	...
	public String getReport() {
		...
		String report = "<html><body><h1>Entropy Report</h1>"
						+ "<hr>"
						+ "<br>"
						+ "<br><h2>Results:</h2>"
						+ "Entropy: " + f.format(value)
						+ "</body></html>";
